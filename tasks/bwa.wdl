...
task bamStat {
    ...
    command <<<
        source activate base
        conda activate bwa_env
        touch ~{tag + ".insert_size_metrics.txt"}
        touch ~{tag + ".insert_size_histogram.pdf"}
...
CODE
        ...
        I=~{bam[0]} \
        O=~{tag + ".insert_size_metrics.txt"} \
        H=~{tag + ".insert_size_histogram.pdf"} \
        M=0.5
    >>>
    output {
        String sample = tag
        ...
        File insert_size_metrics = tag + ".insert_size_metrics.txt"
        File insert_size_histogram = tag + ".insert_size_histogram.pdf"
...
workflow loose_mapping_metrics {
    ...
    output {
        ...
        File insert_size_metrics = bamStat.insert_size_metrics
    }
}
